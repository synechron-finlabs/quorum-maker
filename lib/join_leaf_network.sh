...
function readParameters() {
    ...
    if [[ -z "$sNode" && -z "$pMainIp" && -z "$mgoPort" && -z "$mRPort" && -z "$pCurrentIp" && -z "$rPort" && -z "$wPort" && -z "$cPort" && -z "$raPort" && -z "$tgoPort" && -z "$wsPort" ]]; then
        return
    fi
...
function readInputs(){  
    ...
    if [ -z "$NON_INTERACTIVE" ]; then   
        ...
        getInputWithDefault 'Please enter RPC Port of existing node' "" mRPort $GREEN
...
function copyScripts(){
    ...
    cp lib/slave/pre_start_check_template_leaf.sh ${sNode}/node/pre_start_check.sh
...
function createSetupConf() {
    ...
    echo 'MASTER_RPC_PORT='${mRPort} >> ${sNode}/setup.conf
